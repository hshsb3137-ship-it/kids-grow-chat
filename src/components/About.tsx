...
import { DEFAULT_ABOUT, DEFAULT_ABOUT_FEATURES } from "@/lib/about-defaults";

const LUCIDE: Record<string, React.ComponentType<{ className?: string }>> = {
  Smile, Brain, Sparkles, Heart,
};

function FeatureIcon({ icon }: { icon: string }) {
  const Cmp = LUCIDE[icon];
  if (Cmp) return <Cmp className="h-6 w-6" />;
  return <span className="text-xl leading-none">{icon}</span>;
}

export function About() {
  const { data } = useQuery({ queryKey: ["site_content"], queryFn: fetchSiteContent });
  const about: AboutContent = { ...DEFAULT_ABOUT, ...(data?.about ?? {}) };
  const rawFeatures: AboutFeature[] = Array.isArray(data?.about_features) && data.about_features.length
    ? data.about_features
