...
export function CartDrawer() {
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <>
          <motion.div key="backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={closeCart}
          ...
          <motion.aside key="drawer" initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }}
            ...
            <header className="flex items-center justify-between border-b border-border px-5 py-4">
              ...
              <button onClick={closeCart} aria-label="Close cart" className="grid h-9 w-9 place-items-center rounded-full bg-accent hover:bg-primary/20">
                <X className="h-4 w-4" />
              </button>
            </header>
            ...
            {items.length === 0 ? (
              <div className="flex flex-1 flex-col items-center justify-center gap-4 px-6 text-center">
                ...
                <Link to="/shop" onClick={closeCart} className="rounded-full bg-gradient-primary px-6 py-3 text-sm font-bold text-primary-foreground shadow-pop">
                  Browse Products
                </Link>
              </div>
            ) : (
              <>
                <ul className="flex-1 space-y-3 overflow-y-auto px-5 py-4">
                  <AnimatePresence initial={false}>
                    {items.map((item) => (
                      <motion.li key={item.id} layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: 40 }}
                        className="flex gap-3 rounded-2xl border border-border bg-card p-3 shadow-soft">
                        <img src={item.image} alt={item.name} className="h-20 w-20 flex-none rounded-xl object-cover" loading="lazy" />
                        <div className="flex flex-1 flex-col">
                          ...
                          <div className="mt-auto flex items-center justify-between">
                            <div className="inline-flex items-center rounded-full border border-border bg-background">
                              <button onClick={() => setQuantity(item.id, item.quantity - 1)} aria-label="Decrease quantity"
                                className="grid h-8 w-8 place-items-center rounded-full hover:bg-accent">
                                <Minus className="h-3.5 w-3.5" />
                              </button>
                              <span className="w-8 text-center text-sm font-bold">{item.quantity}</span>
                              <button onClick={() => setQuantity(item.id, item.quantity + 1)} aria-label="Increase quantity"
                                className="grid h-8 w-8 place-items-center rounded-full hover:bg-accent">
                ...
                <footer className="border-t border-border bg-background/95 px-5 py-4">
                  ...
                  <a href={whatsappCartUrl(items.map((i) => ({ name: i.name, quantity: i.quantity, price: i.price })))}
                    target="_blank" rel="noreferrer" onClick={handleOrder}
                  ...
                  <button onClick={clear} className="mt-2 w-full rounded-full px-5 py-2 text-xs font-semibold text-muted-foreground hover:text-destructive">
                    Clear cart
                  </button>
                </footer>
              </>
